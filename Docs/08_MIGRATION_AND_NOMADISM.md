...
**Version:** 1.1 (consistency pass 2026-09-11)  
**Gameplay climate contract:** `Docs/DESIGN_CLIMATE_AND_MIGRATION_PRESSURE.md` (Stage 1.5)  
**Depends on:**  
...
The strategic map is not completely static **in value**.

Environmental **conditions** change over time. On the worldbuilding level this is “living geography”: the same place can become more or less suitable for life, pasture, and settlement.

**Stage 1.5 gameplay contract** (`DESIGN_CLIMATE_AND_MIGRATION_PRESSURE.md`):

* geographic regions are **fixed** for the match;
* region boundaries **do not overlap**;
* exactly three climate states: `COLD` / `FAVORABLE` / `DRY`;
* the seasonal system changes **region state**, not region geometry;
* `TRANSITION` is not a fourth gameplay state in the current model;
* horses and soft resource pressure respond to climate state.

Older phrases such as “zones move across the map” describe the **lore feeling** that favorable conditions shift over time. They do **not** require the implementation to translate mesh circles or region boundaries during a match.

Conceptually the world still contains:

* favorable periods;
* seasonal transition timing (not a separate state enum);
* drought / heat (`DRY`);
* severe cold (`COLD`);
* changing water and vegetation suitability;
* seasonal animal / horse availability;
* regional environmental hazards.

Region **shape** may later follow terrain (polygon, mask). Shape is map design. **Movement of boundaries is not the current model.**

The goal remains: the world feels like a **living environmental system**, not a board where every resource tile is permanently equal — while keeping Stage 1.5 implementation on fixed regions + changing state.
...
A player who knows that a **region’s climate state** will soon become more favorable (or that a currently favorable region will worsen) can prepare:
...
## 25.1. Gameplay note (2026-09-11)

This foundation document remains valid as philosophy. Implementation details for the next vertical slices are owned by:

* `Docs/DESIGN_CLIMATE_AND_MIGRATION_PRESSURE.md`
* `Docs/STAGE_1_5_GAMEPLAY.md`
* `Docs/CURRENT_STATE.md`

Do not treat early “moving zone” language in this file as an override of the fixed-region Stage 1.5 contract.
...
* changing environmental **conditions** (fixed regions, changing climate state in Stage 1.5);
* resource availability affected by environment;
* mobile settlement for relevant factions;
* mobile Towers where appropriate;
* horse / animal economy;
* raid and plunder;
* territorial competition driven by environmental pressure.

Exact implementation order is owned by Stage 1.5 design/contracts, not this foundation document.

---

# 28. OPEN QUESTIONS

Some questions remain intentionally unresolved at the world-foundation layer and belong in dedicated Game Design / Stage 1.5 documents.

### Environmental system

* How predictable are climate state cycles?
...
Docs/STAGE_1_5_GAMEPLAY.md
Docs/CURRENT_STATE.md
GameDesign/ (if present)
```

When this file conflicts with the Stage 1.5 gameplay climate contract on **current match implementation**, the Stage 1.5 contract wins for implementation scope.
...
Resources change in value.

Animals become available or unavailable.
