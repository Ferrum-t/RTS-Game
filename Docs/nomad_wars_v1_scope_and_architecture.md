...
> Использовать: прикладывать целиком (или релевантный раздел) в начале новой сессии
> с любой моделью (Claude / ChatGPT / Grok), плюс отдельно — короткий "текущий статус"
> (что сделано с последнего сохранения этого файла).
...
1. **Cleanup** — убрать диагностические print'ы, dead code (дёшево, делаем сразу).
2. **Order abstraction** (см. 3.1) — разблокирует Raid и мобильные здания разом.
3. **Стат-резолвер base×tier×deployment** (см. 3.3) — до реализации мобильных
   зданий, иначе характеристики придётся переписывать.
4. **Спайк: физика DEPLOYED↔MOBILE в Godot** (см. 3.4) — маленький изолированный
   прототип на одном здании, прежде чем фиксировать контракт компонентов.
5. **DeploymentComponent + NavigationBakeService register/unregister** (3.2, 3.5).
6. **Мобильный Town Center** — первая полная реализация фичи на основе 2–5.
