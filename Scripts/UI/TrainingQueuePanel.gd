...
func _process(_delta: float) -> void:
	_update_from_selection()


func _update_from_selection() -> void:
	...
	if sm == null or not sm.has_method("get_selected_mobile_buildings"):
		...
		_building = null
		return
	var buildings: Array = sm.get_selected_mobile_buildings()
	var found: Node = null
	for b in buildings:
...
func _refresh_display() -> void:
	if _building == null or not is_instance_valid(_building):
		return
	var name_s: String = str(_building.name)
	if _building is TownCenter:
		_title.text = "TC — " + name_s
	elif _building is Barracks:
		_title.text = "Barracks — " + name_s
	else:
		_title.text = name_s
	...
	elif "train_progress" in _building:
		progress = float(_building.train_progress)
	_progress.value = clampf(progress, 0.0, 1.0)

	var pipeline: int = 0
	var labels: Array = []
	if _building.has_method("get_train_pipeline_count"):
		pipeline = int(_building.get_train_pipeline_count())
	if _building.has_method("get_train_queue_labels"):
		labels = _building.get_train_queue_labels()
	if labels.is_empty():
		_queue_label.text = "Queue: %d/5" % pipeline
	else:
		_queue_label.text = "Queue: " + ", ".join(labels)

	var busy: bool = pipeline > 0 or progress > 0.0
...
func _on_cancel_last() -> void:
	if _building == null or not is_instance_valid(_building):
		return
	if _building.has_method("cancel_train_last"):
...
func _on_cancel_all() -> void:
	if _building == null or not is_instance_valid(_building):
		return
	if _building.has_method("cancel_train_all"):
